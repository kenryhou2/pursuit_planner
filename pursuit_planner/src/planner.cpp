...
struct Waypoint {
    ...
    bool is_detour;  // true if this waypoint was created as a detour around an obstacle
...
struct PlannerState {
    ...
    PlannerState() : initialized(false), currentWaypointIdx(0), waypointRadius(4.0), sensorRadius(4.0) {}
};
...
// Find detour waypoints around a blocked waypoint
// Returns a pair of waypoints that go around the obstacle while staying within visibility range
vector<Waypoint> findDetourWaypoints(int fromX, int fromY, int blockedX, int blockedY, 
    ...
    // Detour distance - enough to go around but within visibility
    double detour_dist = visibility_range * 0.5;  // Half the visibility range
    
    // Try both sides (positive and negative perpendicular)
    ...
    auto isDetourValid = [&](int dx, int dy) -> bool {
        ...
        // Distance from robot (fromX, fromY) to the detour location
        double dist_to_detour = euclideanDist(fromX, fromY, dx, dy);
        // Check if blocked by dynamic obstacle within sensor range
        if (globalState.compactObstacles.timesteps.size() > 0  &&
            (globalState.sensorRadius <= 0.0 || dist_to_detour <= globalState.sensorRadius)) {
            for (int t = estimated_arrival - 2; t <= estimated_arrival + 3; t++) { //window of 6 time steps around arrival
    ...
    // Pick the better side (prefer the one closer to the goal) //goal is the next waypoint.
    ...
    if (side1_valid && side2_valid) {
        // Both valid, pick closer to goal
        ...
        // Neither side is valid, try smaller detour
        ...
        if (side1_valid) {
            ...
            return detours;  // No valid detour found
        }
    }
    
    // Create the detour waypoint
...
pair<int,int> segmentAStarSearch(int startX, int startY, int goalX, int goalY, 
    ...
    if (!found) {
        cout << "  Segment A* failed: iterations=" << iterations << "/" << max_iterations 
             << ", open_set empty=" << open_set.empty() 
             << ", time_limit=" << time_limit 
             << ", goal=(" << goalX << "," << goalY << ")" << endl;
...
)
{
    ...
    globalState.sensorRadius = WAYPOINT_DISTANCE;
    ...
    if (curr_time == 0) {
        ...
        // Always target the final position of the target
        ...
        // Write initial waypoints to file
    ...
    // Check if we've reached or passed the current waypoint
    if (globalState.currentWaypointIdx < (int)globalState.waypoints.size()) {
        ...
        // Move to next waypoint only if we're actually AT the waypoint (same cell)
        if (robotposeX == curr_wp.x && robotposeY == curr_wp.y) {
            globalState.currentWaypointIdx++;
        }
    }
    
    // Check if current target waypoint is blocked - if so, create detour waypoints
    ...
    bool blocked = false;
    // Distance from robot to this waypoint *right now*
    double dist_to_wp = euclideanDist(robotposeX, robotposeY, target_wp.x, target_wp.y);

    // Only check for dynamic obstacles if within sensor range
    if (globalState.compactObstacles.timesteps.size() > 0 &&
        (globalState.sensorRadius <= 0.0 || dist_to_wp <= globalState.sensorRadius)) { 
    ...
    if (blocked && !target_wp.is_detour) {
        // Find the next waypoint after this one for direction
        ...
        if (!detours.empty()) {
            // Replace the blocked waypoint with detour waypoint
            ...
            // Replace the blocked waypoint with the detour
            globalState.waypoints[target_wp_idx] = detours[0];
            
            // Update waypoints file with new detour
            ofstream wp_file("../output/waypoints.txt");
            if (wp_file.is_open()) {
                for (size_t i = 0; i < globalState.waypoints.size(); i++) {
                    wp_file << globalState.waypoints[i].x << "," 
            ...
            // No valid detour found, mark this as detour to skip re-checking
            cout << "  WP" << target_wp_idx << " blocked, no valid detour found, skipping" << endl;
            globalState.waypoints[target_wp_idx].is_detour = true;  // Mark to avoid infinite loop
            globalState.currentWaypointIdx++;  // Skip this waypoint
        }
    }
    ...
    // Allow much more time for larger maps - use at least 5x estimated time plus a base buffer
    int segment_time_limit = curr_time + MAX(estimated_segment_time * 5 + 100, globalState.maxTimeHorizon);
